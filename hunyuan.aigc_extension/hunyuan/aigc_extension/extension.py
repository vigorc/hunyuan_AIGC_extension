...
class MyExtension(omni.ext.IExt):
    ...
    def on_startup(self, ext_id):
        ...
        # Generation mode: 0 = Cloud API, 1 = Local Server
        self._generation_mode = 0
        
        # Cloud API configuration (defaults, will be overridden by config.json)
        self._cloud_api_url = "https://api.ai3d.cloud.tencent.com/v1/ai3d"
        self._cloud_api_key = ""
        self._cloud_gen_type = 0  # 0 = Image to 3D, 1 = Text to 3D
        self._text_prompt = ""
        ...
        # Hunyuan local server configuration (may be overridden by config.json)
        if not hasattr(self, '_hunyuan_server_url'):
        ...
        with self._window.frame:
            ...
            with main_scroll:
                with ui.VStack(spacing=10):
                    ...
                    with ui.HStack(height=30):
                        ui.Label("Generation Mode:", width=110)
                        self._mode_combo = ui.ComboBox(0, "Cloud API (Tencent AI3D)", "Local Server")
                        self._mode_combo.model.add_item_changed_fn(self._on_generation_mode_changed)
                    
                    # ===== Cloud API Configuration Frame =====
                    self._cloud_api_frame = ui.VStack(spacing=5)
                    with self._cloud_api_frame:
                        # API Key (show masked if loaded from config)
                        with ui.HStack(height=30):
                            ui.Label("API Key:", width=110)
                            self._api_key_field = ui.StringField()
                            self._api_key_field.model.set_value(self._mask_api_key(self._cloud_api_key))
                        
                        # Generation type (Image to 3D / Text to 3D)
                        with ui.HStack(height=30):
                            ui.Label("Input Type:", width=110)
                            self._cloud_gen_type_combo = ui.ComboBox(0, "Image to 3D", "Text to 3D")
                            self._cloud_gen_type_combo.model.add_item_changed_fn(self._on_cloud_gen_type_changed)
                        
                        # Selected image for cloud generation (Image to 3D)
                        self._cloud_image_frame = ui.HStack(height=30)
                        with self._cloud_image_frame:
                            ui.Label("Selected Image:", width=110)
                            self._selected_image_label = ui.Label("None selected", word_wrap=True)
                        
                        # Text prompt input (Text to 3D)
                        self._cloud_text_frame = ui.VStack(spacing=3)
                        with self._cloud_text_frame:
                            ui.Label("Text Prompt:", height=20)
                            self._text_prompt_field = ui.StringField(height=50, multiline=True)
                            self._text_prompt_field.model.set_value("")
                        
                        # Initially hide text prompt (show image mode by default)
                        self._cloud_text_frame.visible = False
                        
                        # Test cloud API button
                        ui.Button("Test Cloud API", clicked_fn=self._test_cloud_api_connection, height=28)
                    
                    # ===== Local Server Configuration Frame =====
                    self._local_server_frame = ui.VStack(spacing=5)
                    with self._local_server_frame:
                        # Selected image for local generation
                        with ui.HStack(height=30):
                            ui.Label("Selected Image:", width=110)
                            self._local_selected_image_label = ui.Label("None selected", word_wrap=True)
                        
                        # Server URL
                        with ui.HStack(height=30):
                            ui.Label("Server URL:", width=110)
                            self._server_url_field = ui.StringField()
                            self._server_url_field.model.set_value(self._hunyuan_server_url)
                        
                        # Test server button
                        ui.Button("Test Server Connection", clicked_fn=self._test_server_connection, height=28)
                    
                    # Initially hide local server frame (cloud API is default)
                    self._local_server_frame.visible = False
                    
                    # Separator
                    ui.Separator(height=2)
                    
                    # ===== Common Options =====
                    ui.Label("Options", height=25, style={"font_size": 16})
    ...
    def _mask_api_key(self, api_key):
        """Mask API key for display, e.g. 'sk-hx0Rf3...mMQErDM'"""
        if not api_key:
            return ""
        key = api_key.strip()
        if len(key) <= 10:
    ...
    def _get_real_api_key(self):
        """Get the real API key: if UI shows masked value, use the one from config; otherwise use UI input."""
        ui_value = self._api_key_field.model.get_value_as_string().strip()
        
        if not ui_value:
            return ""
        
        # Check if the UI value is the masked version of the stored key
        if self._cloud_api_key and ui_value == self._mask_api_key(self._cloud_api_key):
            return self._cloud_api_key
        
        # User typed a new key directly
        if "..." not in ui_value:
            # It's a full new key, save it
            if ui_value != self._cloud_api_key:
                self._cloud_api_key = ui_value
                self._save_config()
                # Update display to masked version
                self._api_key_field.model.set_value(self._mask_api_key(ui_value))
            return ui_value
        
        # Fallback to stored key
        return self._cloud_api_key if self._cloud_api_key else ui_value
        ...
        try:
            if self._config_path.exists():
                ...
                if api_key:
                    ...
                    print(f"[hunyuan.aigc_extension] API Key loaded from config.json (first 8 chars: {api_key[:8]}...)")
                
                # Load cloud API URL
                cloud_url = config.get('cloud_api_url', '').strip()
                if cloud_url:
                    self._cloud_api_url = cloud_url
                
                # Load local server URL
                local_url = config.get('local_server_url', '').strip()
                if local_url:
                    self._hunyuan_server_url = local_url
                
                print("[hunyuan.aigc_extension] Config loaded from config.json")
            else:
                # Create default config file
                self._save_config()
                print("[hunyuan.aigc_extension] Default config.json created")
        except Exception as e:
            print(f"[hunyuan.aigc_extension] Failed to load config.json: {e}")
    ...
    def _save_config(self):
        ...
        try:
            config = {
                ...
                "local_server_url": self._hunyuan_server_url
            }
            with open(self._config_path, 'w', encoding='utf-8') as f:
                json.dump(config, f, indent=4, ensure_ascii=False)
            print("[hunyuan.aigc_extension] Config saved to config.json")
        except Exception as e:
            print(f"[hunyuan.aigc_extension] Failed to save config.json: {e}")
    
    def _on_generation_mode_changed(self, model, item):
        """Callback when generation mode changes (Cloud API / Local Server)"""
        index = model.get_item_value_model().get_value_as_int()
        self._generation_mode = index
        
        # Toggle visibility of panels
        if hasattr(self, '_cloud_api_frame') and hasattr(self, '_local_server_frame'):
            self._cloud_api_frame.visible = (index == 0)
            self._local_server_frame.visible = (index == 1)
        
        mode_name = "Cloud API" if index == 0 else "Local Server"
        print(f"[hunyuan.aigc_extension] Generation mode changed to: {mode_name}")
    
    def _on_cloud_gen_type_changed(self, model, item):
        """Callback when cloud generation type changes (Image to 3D / Text to 3D)"""
        index = model.get_item_value_model().get_value_as_int()
        self._cloud_gen_type = index
        
        # Toggle visibility: Image frame vs Text frame
        if hasattr(self, '_cloud_image_frame') and hasattr(self, '_cloud_text_frame'):
            self._cloud_image_frame.visible = (index == 0)
            self._cloud_text_frame.visible = (index == 1)
        
        gen_type_name = "Image to 3D" if index == 0 else "Text to 3D"
        print(f"[hunyuan.aigc_extension] Cloud generation type changed to: {gen_type_name}")
            ...
            with ui.HStack(height=80, spacing=10):
                ...
                with ui.VStack(width=80):
                    def select_for_generation():
                        ...
                        if hasattr(self, '_local_selected_image_label'):
                            self._local_selected_image_label.text = filename
                    ...
                    def remove_image():
                        try:
                            ...
                            if self._selected_image_path == image_path:
                                ...
                                if hasattr(self, '_local_selected_image_label'):
                                    self._local_selected_image_label.text = "None selected"
                            # Rebuild UI
    ...
    def _test_cloud_api_connection(self):
        ...
        try:
            api_key = self._get_real_api_key()
            cloud_url = self._cloud_api_url
            
            if not api_key:
                self._update_status("Please enter API Key or configure in config.json", "red")
                return
            
            self._update_status("Testing Cloud API connection...", "orange")
            
            # Try a simple query request to test the connection
            headers = {
                'Authorization': api_key,
                'Content-Type': 'application/json'
            }
            
            # Send a query with a dummy job ID to check if the API is reachable
            payload = json.dumps({"JobId": "test_connection"})
            response = requests.post(f"{cloud_url}/query", headers=headers, data=payload, timeout=10)
            ...
            if response.status_code == 200:
                ...
                if 'Response' in result:
                    if 'Error' in result['Response']:
                        ...
                        if error_code in ['AuthFailure', 'AuthFailure.SignatureFailure']:
                            self._update_status("API Key invalid, please check", "red")
                        else:
                            # API is reachable, just the test job ID doesn't exist
                            self._update_status("Cloud API connection successful!", "green")
                    else:
                        self._update_status("Cloud API connection successful!", "green")
                else:
                    self._update_status("Cloud API response format unexpected", "orange")
            else:
                self._update_status(f"Cloud API error: HTTP {response.status_code}", "red")
                
        except requests.exceptions.ConnectionError:
            self._update_status("Cannot connect to Cloud API", "red")
        except requests.exceptions.Timeout:
            self._update_status("Cloud API connection timeout", "red")
        except Exception as e:
            self._update_status(f"Cloud API test failed: {str(e)}", "red")
    ...
    def _generate_3d_model(self):
        """Generate 3D model based on current mode selection"""
        if self._generation_mode == 0:
            # Cloud API mode
            self._generate_3d_model_cloud()
        else:
            # Local Server mode
            self._generate_3d_model_local()
    
    def _generate_3d_model_local(self):
        """Generate 3D model from selected image using local server"""
    ...
    def _generate_3d_model_cloud(self):
        """Generate 3D model using Cloud API"""
        api_key = self._get_real_api_key()
        ...
        cloud_gen_type = self._cloud_gen_type_combo.model.get_item_value_model().get_value_as_int()
        ...
        if cloud_gen_type == 0:
            ...
            # Text to 3D mode
            text_prompt = self._text_prompt_field.model.get_value_as_string().strip()
            if not text_prompt:
                self._update_status("Please enter a text prompt", "red")
                return
        
        # Disable generate button during processing
        self._generate_btn.enabled = False
        
        # Start cloud generation in a separate thread
        thread = threading.Thread(target=self._generate_3d_model_cloud_async)
        thread.daemon = True
        thread.start()
    ...
    # ============================================================================
    # Cloud API Generation Methods
    # ============================================================================
    
    def _cloud_img_to_base64(self, img_path):
        """Convert image file to base64 string for Cloud API (raw binary base64)"""
        try:
            with open(img_path, "rb") as f:
                base64_str = base64.b64encode(f.read()).decode("utf-8")
            return base64_str
        except FileNotFoundError:
            print(f"[hunyuan.aigc_extension] Image file not found: {img_path}")
            return None
        except Exception as e:
            print(f"[hunyuan.aigc_extension] Image to base64 failed: {e}")
            return None
    
    def _cloud_api_submit(self, api_key, cloud_url, payload):
        """Submit a generation task to Cloud API"""
        headers = {
            'Authorization': api_key,
            'Content-Type': 'application/json'
        }
        
        print(f"[hunyuan.aigc_extension] Submitting to: {cloud_url}/submit")
        print(f"[hunyuan.aigc_extension] API Key (first 8 chars): {api_key[:8]}...")
        
        response = requests.post(f"{cloud_url}/submit", headers=headers, data=json.dumps(payload), timeout=30)
        
        # Handle HTTP errors with specific messages
        if response.status_code == 401:
            raise Exception("API Key authentication failed (401 Unauthorized). Please check your API Key.")
        elif response.status_code == 403:
            raise Exception("Access denied (403 Forbidden). API Key may not have permission.")
        ...
        if 'Error' in response_data:
            ...
            raise Exception(f"API Error [{error_info.get('Code', 'Unknown')}]: {error_info.get('Message', 'Unknown error')}")
        
        job_id = response_data.get('JobId')
        if not job_id:
            raise Exception("No JobId returned from Cloud API")
        
        return job_id
    
    def _cloud_api_query(self, api_key, cloud_url, job_id):
        """Query task status from Cloud API"""
        headers = {
            'Authorization': api_key,
            'Content-Type': 'application/json'
        }
        payload = json.dumps({"JobId": job_id})
        response = requests.post(f"{cloud_url}/query", headers=headers, data=payload, timeout=15)
        
        if response.status_code == 401:
            raise Exception("API Key authentication failed (401)")
        elif response.status_code != 200:
            raise Exception(f"Query HTTP {response.status_code}")
        
        result = response.json()
        return result.get('Response', {})
    
    def _cloud_download_model(self, url, save_path):
        """Download 3D model file from URL"""
        try:
            print(f"[hunyuan.aigc_extension] Downloading model: {save_path}")
            ...
            print(f"[hunyuan.aigc_extension] Download complete: {save_path} ({file_size:.2f} MB)")
            return True
        except Exception as e:
            print(f"[hunyuan.aigc_extension] Download failed: {e}")
            return False
    
    def _generate_3d_model_cloud_async(self):
        """Async method to generate 3D model via Cloud API"""
        try:
            api_key = self._get_real_api_key()
            cloud_url = self._cloud_api_url
            cloud_gen_type = self._cloud_gen_type_combo.model.get_item_value_model().get_value_as_int()
            ...
            if cloud_gen_type == 0:
                # Image to 3D
                self._update_status("Converting image to base64...", "blue")
                image_base64 = self._cloud_img_to_base64(self._selected_image_path)
                if not image_base64:
                    self._update_status("Failed to process image", "red")
                    self._generate_btn.enabled = True
                    return
                payload["ImageBase64"] = image_base64
                self._update_status("Submitting image to Cloud API...", "blue")
            else:
                # Text to 3D
                text_prompt = self._text_prompt_field.model.get_value_as_string().strip()
                payload["Text"] = text_prompt
                self._update_status("Submitting text prompt to Cloud API...", "blue")
            
            # Submit task
            try:
                job_id = self._cloud_api_submit(api_key, cloud_url, payload)
            except Exception as e:
                self._update_status(f"Submit failed: {str(e)}", "red")
                self._generate_btn.enabled = True
                return
            
            self._update_status(f"Task submitted (JobId: {job_id[:12]}...)", "blue")
            print(f"[hunyuan.aigc_extension] Cloud API JobId: {job_id}")
            
            # Poll for completion
            self._poll_cloud_generation_status(api_key, cloud_url, job_id)
            
        except requests.exceptions.ConnectionError:
            self._update_status("Cannot connect to Cloud API", "red")
            self._generate_btn.enabled = True
        except requests.exceptions.Timeout:
            self._update_status("Cloud API request timeout", "red")
            self._generate_btn.enabled = True
        except Exception as e:
            self._update_status(f"Cloud generation failed: {str(e)}", "red")
            self._generate_btn.enabled = True
    
    def _poll_cloud_generation_status(self, api_key, cloud_url, job_id):
        """Poll Cloud API generation status until completion"""
        try:
            ...
            while attempt < max_attempts:
                try:
                    response_data = self._cloud_api_query(api_key, cloud_url, job_id)
                except Exception as e:
                    self._update_status(f"Query failed: {str(e)}", "red")
                    attempt += 1
                    time.sleep(5)
                    continue
                ...
                if status == 'DONE':
                    self._update_status("Cloud generation completed! Downloading...", "green")
                    
                    # Get result files
                    result_files = response_data.get('ResultFile3Ds', [])
                    if not result_files:
                        self._update_status("No model files in result", "red")
                        break
                    
                    # Download GLB file (prefer GLB format)
                    downloaded = False
                    for file_info in result_files:
                        ...
                        if file_type == 'GLB' and file_url:
                            ...
                            filename = f"cloud_model_{job_id[:8]}_{timestamp}.glb"
                            filepath = str(self._models_dir / filename)
                            
                            self._update_status(f"Downloading GLB model...", "blue")
                            if self._cloud_download_model(file_url, filepath):
                                # Add to generated models list
                                self._generated_models.append(filepath)
                                
                                # Handle auto-load or add to UI
                                if self._auto_load_to_stage:
                                    ...
                                    self._update_status(f"Model downloaded and loading to stage: {filename}", "blue")
                                else:
                                    self._pending_ui_updates.append({
                                        'type': 'add_model',
                                        'model_path': filepath,
                                    ...
                                    self._update_status(f"Model downloaded: {filename}", "green")
                                
                                downloaded = True
                            else:
                                self._update_status("Failed to download model", "red")
                            break
                    
                    if not downloaded:
                        # Try to download first available file if no GLB found
                        for file_info in result_files:
                            file_type = file_info.get('Type', '')
                            file_url = file_info.get('Url', '')
                            if file_url:
                                timestamp = int(time.time())
                                ext = '.glb' if file_type == 'GLB' else '.zip' if file_type == 'OBJ' else f'.{file_type.lower()}'
                                filename = f"cloud_model_{job_id[:8]}_{timestamp}{ext}"
                                filepath = str(self._models_dir / filename)
                        ...
                        if not downloaded:
                            self._update_status("No downloadable model files found", "red")
                    break
                    
                elif status == 'FAILED':
                    error_msg = response_data.get('ErrorMsg', 'Unknown error')
                    self._update_status(f"Cloud generation failed: {error_msg}", "red")
                    break
                    
                elif status in ['PROCESSING', 'RUNNING']:
                    progress = response_data.get('Progress', '')
                    if progress:
                        self._update_status(f"Processing: {progress}...", "blue")
                    else:
                        self._update_status(f"Status: {status}...", "blue")
                    
                elif status in ['SUBMITTED', 'PENDING', 'WAITING']:
                    self._update_status(f"Task queued ({status}), waiting...", "blue")
            ...
            if attempt >= max_attempts:
                self._update_status("Cloud generation timeout (15 min)", "red")
                
        except Exception as e:
            self._update_status(f"Cloud status check failed: {str(e)}", "red")
        finally:
            self._generate_btn.enabled = True
